...
contract BaseERC1155CreatorTest is Test {
    ...
    function mintWithCreator(address[] memory tos, uint256[] memory amounts, string[] memory uris)
        internal
        returns (uint256[] memory)
    {
        vm.prank(creator);
        uint256[] memory tokenIds = creatorContract().mintBaseNew(tos, amounts, uris);
        ...
        if (tokenIds.length > 0) {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                ...
                assertMintWithCreator(tokenIds[i], tos.length == 1 ? tos[0] : tos[i], amounts[i], uri);
            }
            return tokenIds;
        }
    ...
    function mintWithCreator(address[] memory tos, uint256[] memory amounts, string memory uri)
        internal
        returns (uint256)
    {
    ...
    function mintWithCreator(address[] memory tos, uint256 amount, string memory uri) internal returns (uint256) {
        uint256[] memory amounts = new uint256[](1);
        amounts[0] = amount;
        return mintWithCreator(tos, amounts, uri);
    }

    function mintWithCreator(address to, uint256 amount, string memory uri) internal returns (uint256) {
        address[] memory tos = new address[](1);
        tos[0] = to;
        return mintWithCreator(tos, amount, uri);
    }

    function mintWithCreator(address[] memory tos, uint256[] memory amounts) internal returns (uint256) {
        string[] memory uris = new string[](0);
        uint256[] memory tokenIds = mintWithCreator(tos, amounts, uris);
        return tokenIds[0];
    }

    function mintWithCreator(address[] memory tos, uint256 amount) internal returns (uint256) {
        uint256[] memory amounts = new uint256[](1);
        amounts[0] = amount;
        return mintWithCreator(tos, amounts);
    }

    function mintWithCreator(address to, uint256 amount) internal returns (uint256) {
        address[] memory tos = new address[](1);
        tos[0] = to;
        return mintWithCreator(tos, amount);
    }

    function mintWithCreator(address[] memory tos, string memory uri) internal returns (uint256) {
    ...
    function mintBatchWithCreator(address to, uint256 amount) internal returns (uint256[] memory) {
        address[] memory tos = new address[](1);
        tos[0] = to;
    ...
    function mintWithExtension(address extension, address[] memory tos, uint256[] memory amounts, string[] memory uris)
        internal
        returns (uint256[] memory)
    {
        vm.prank(creator);
        uint256[] memory tokenIds = IERC1155Extension(extension).mint(tos, amounts, uris);
        ...
        if (tokenIds.length > 0) {
            for (uint256 i = 0; i < tokenIds.length; i++) {
                ...
                assertMintWithExtension(extension, tokenIds[i], tos.length == 1 ? tos[0] : tos[i], amounts[i], uri);
            }
            return tokenIds;
        }
    ...
    function mintWithExtension(address extension, address[] memory tos, uint256[] memory amounts, string memory uri)
        internal
        returns (uint256)
    {
        string[] memory uris = new string[](1);
        uris[0] = uri;
        uint256[] memory tokenIds = mintWithExtension(extension, tos, amounts, uris);
        return tokenIds[0];
    }

    function mintWithExtension(address extension, address[] memory tos, uint256 amount, string memory uri)
        internal
        returns (uint256)
    {
        uint256[] memory amounts = new uint256[](1);
    ...
    function mintWithExtension(address extension, address to, uint256 amount, string memory uri)
        internal
        returns (uint256)
    {
        address[] memory tos = new address[](1);
    ...
    function mintWithExtension(address extension, address[] memory tos, uint256[] memory amounts)
        internal
        returns (uint256)
    {
        string[] memory uris = new string[](0);
    ...
    function mintWithExtension(address extension, address[] memory tos, uint256 amount) internal returns (uint256) {
        uint256[] memory amounts = new uint256[](1);
        amounts[0] = amount;
        return mintWithExtension(extension, tos, amounts);
    }

    function mintWithExtension(address extension, address to, uint256 amount) internal returns (uint256) {
        address[] memory tos = new address[](1);
        tos[0] = to;
        return mintWithExtension(extension, tos, amount);
    }

    function mintWithExtension(address extension, address[] memory tos, string memory uri) internal returns (uint256) {
    ...
    function mintWithExtension(address extension, address[] memory tos) internal returns (uint256) {
    ...
    function mintBatchWithExtension(address extension, address to, uint256 amount)
        internal
        returns (uint256[] memory)
    {
        address[] memory tos = new address[](1);
    ...
    function assertMintWithCreator(uint256 tokenId, address to, uint256 amount, string memory uri) internal {
    ...
    function assertMintWithExtension(address extension, uint256 tokenId, address to, uint256 amount, string memory uri)
        internal
    {
    ...
    function assertMint(uint256 tokenId, address to, uint256 amount, string memory uri) internal {
